...
class Library {
    ...
    constructor() {
        ...
        if (
            window.location.search.includes("save=1") &&
            window.location.search.includes("tabID=")
        ) {
            let offset = window.location.search.indexOf("tabID=") + 6;
            let tabID = window.location.search.substr(
                offset,
                window.location.search.length - offset
            );
            this.save(tabID);
        } else {
            this.refreshSnapshots().then(
                function() {
    ...
    attachHandlers() {
        for (let element of document.querySelectorAll(
            "#slideshow > .screenshot-actions > div"
        )) {
            // screenshot actions
            element.addEventListener(
                "click",
                this.screenshotActionClickHandler.bind(this, element)
            );
        }

        for (let element of document.querySelectorAll(
            "#slideshow > .screenshot-navigation > i"
        )) {
            // screenshot navigation
            element.addEventListener(
                "click",
                this.screenshotNavigationClickHandler.bind(this, element)
            );
        }

        document
            .querySelector("#memory-limit > i.fa-exclamation-circle")
            .addEventListener("click", function() {
                window.open(
                    browser.extension.getURL("/web-resources/html/options.html")
                );
            });
    }
                    ...
                    function(items) {
                        if (typeof items[this.storageKeys.options] !== "string")
                            ...
                            let options = JSON.parse(
                                items[this.storageKeys.options]
                            );
        ...
        return new Promise(
            function(resolve, reject) {
                browser.storage.local.get(this.storageKeys.snapshots).then(
                    function(items) {
                        if (
                            items[this.storageKeys.snapshots].constructor ===
                            Array
                        )
     ...
     */
    refreshSlideshow() {
        let currentScreenshotNumber = document.getElementById(
                "current-screenshot-number"
            ),
            totalScreenshotNumber = document.getElementById(
                "total-screenshot-number"
            );

        if (this.snapshots.length > 0) {
            this.elements.slideshow.className = "";
            this.setSlideImage(this.snapshots.length - 1);
            this.elements.slideImage.dataset.storageIndex =
                this.snapshots.length - 1;
            currentScreenshotNumber.innerText =
                this.snapshots.length - 1 == 0 ? 1 : this.snapshots.length;
    ...
    deleteSnapshot(index) {
        return new Promise(
            function(resolve) {
                ...
                this.setSnapshots(this.snapshots).then(function() {
                    resolve();
                });
            }.bind(this)
        );
        ...
        return new Promise(
            function(resolve) {
                this.getStorageSnapshots()
                    ...
                    .catch(
                        function() {
                            this.setSnapshots([snapshot]).then(
                                function() {
                                    this.refreshSlideshow();
                                    resolve();
                                }.bind(this)
    ...
    screenshotNavigationClickHandler(element) {
        let currentImageIndex = parseInt(
                document.getElementById("slide-image").dataset.storageIndex
            ),
            slideImage = document.getElementById("slide-image"),
            currentScreenshotNumber = document.getElementById(
                "current-screenshot-number"
            ),
            newImageIndex;
        ...
        if (!element.title.localeCompare("Previous")) {
            ...
            else newImageIndex = currentImageIndex - 1;
        } else {
    ...
    save(tabID) {
        ...
        browser.tabs
            .sendMessage(parseInt(tabID), { message: "save-canvas" }, null)
            .then(
                function(response) {
                    if (
                        response != null &&
