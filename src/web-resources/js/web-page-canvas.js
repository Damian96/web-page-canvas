...
if (typeof WebPageCanvas === "undefined") {
    ...
    class WebPageCanvas {
        ...
        initCanvas() {
            this.canvas.element = document.querySelector(
            ...
            this.canvas.element.onmouseup = function() {
                if (this.canvas.isDrawing) this.saveAction();
        ...
        draw() {
            ...
            for (let i = 0; i < this.canvas.clickX.length; i++) {
                if (typeof this.canvas.clickX[i] === "undefined") continue;

                this.canvas.context.beginPath();

                if (i)
                    this.canvas.context.moveTo(
                        this.canvas.clickX[i - 1],
                        this.canvas.clickY[i - 1]
                    );
                else
                    this.canvas.context.moveTo(
                        this.canvas.clickX[i] - 1,
                        this.canvas.clickY[i]
                    );

                this.canvas.context.lineTo(
                    this.canvas.clickX[i],
                    this.canvas.clickY[i]
                );
        ...
        // 	frame.src   = browser.extension.getURL('/web-resources/html/web-page-canvas.html');
        ...
        adjustCanvas(width = null, height = null) {
            if (
                this.canvas.hasOwnProperty("element") &&
                this.canvas.element != null
            ) {
                console.log("height", this.getMaxHeight());
                if (width != null) this.canvas.element.width = width;
                else this.canvas.element.width = this.getMaxWidth();

                if (height != null) this.canvas.element.height = height;
                else this.canvas.element.height = this.getMaxHeight();
            }
        }
