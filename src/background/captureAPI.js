...
browser.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    if (request.message == "take-snapshot" && request.hasOwnProperty("data")) {
        if (
            sender.tab.hasOwnProperty("id") &&
            request.data.hasOwnProperty("windowHeight") &&
            request.data.hasOwnProperty("pageHeight")
        ) {
            captureObjects[sender.tab.id] = new CaptureAPI(
                sender.tab.id,
                request.data.windowHeight,
                request.data.pageHeight
            );
            captureObjects[sender.tab.id].init();
            captureObjects[sender.tab.id]
                .takeSnapshot(
                    function(sendResponse, snapshots) {
                        var result = [],
                        ...
                        for (let i = 0; i < snapshots.length; i++) {
                            ...
                            if (
                                ...
                                lastSnapshot == 0 ||
                                (i == lastSnapshot &&
                                    captureObject.maxSnapshots % 1 == 0)
                            ) {
                                ...
                                y -=
                                    captureObject.windowHeight -
                                    (captureObject.maxSnapshots % 1) *
                                        captureObject.windowHeight;
                            }
