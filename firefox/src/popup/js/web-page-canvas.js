 ...
 * @prop {number} tabID The browser's ID of the current tab.
...
class webPageCanvasPopup {
    ...
    updateSlideshow() {
        ...
        browser.storage.local.get(this.STORAGEAREAKEY, function (items) {
            ...
            if (items[this.STORAGEAREAKEY] != null && items[this.STORAGEAREAKEY].length > this.localSnapshots.length) {
                ...
                this.reloadSlideshowWithLocalSnapshots();
                browser.storage.local.set({ [this.STORAGEAREAKEY]: this.localSnapshots });
            } else {
                this.clearSlideshow();
            }
    ...
    switcherClickHandler(element, sendMessageToTab = true) {
        ...
        if (element.classList.contains('off')) {
            ...
            browser.tabs.sendMessage(this.tabID, { message: 'init-canvas' });
            ...
            if (sendMessageToTab) {
                ...
                browser.tabs.sendMessage(this.tabID, { message: 'close-canvas' }, function (response) {
    ...
    restoreClickHandler(element) {
        if (this.lastCanvas != null)
            browser.tabs.sendMessage(this.tabID, { message: 'restore-canvas', data: this.lastCanvas });
        element.disabled = true;
    }

    clearClickHandler() {
        if (this.overlayOpen) {
            browser.tabs.sendMessage(this.tabID, { message: 'clear-canvas' });
        }
    }
    ...
    screenshotActionClickHandler(element) {
        ...
        if (slideshow.className == '' && slideImage.src != null) {
            if (element.classList.contains('download-screenshot'))
                ...
                browser.storage.local.get(this.STORAGEAREAKEY, function (slideImage, slideshow, items) {
                    ...
                    if (typeof data == 'object' && data.length > 0) {
                        ...
                        browser.storage.local.set({ [this.STORAGEAREAKEY]: data });
    ...
    saveClickHandler() {
        ...
        browser.tabs.sendMessage(this.tabID, { message: 'save-canvas' }, function (response) {
    ...
    clearScreenshotsClickHandler() {
        ...
        if (this.localSnapshots.length > 0) {
            ...
            browser.storage.local.set({ [this.STORAGEAREAKEY]: null });
            this.localSnapshots = {};
            this.reloadSlideshowWithLocalSnapshots();
    ...
    insertImage(newImageSrc) {
        return new Promise((resolve) => {
            browser.storage.local.get(this.STORAGEAREAKEY, function (newImageSrc, items) {
                ...
                browser.storage.local.set({ [this.STORAGEAREAKEY]: this.localSnapshots }, function (imageBlobURL) {
...
window.onunload = function () {
    background.popupObjects[webPageCanvas.tabID] = JSON.stringify(webPageCanvas);
};

window.onload = function () {

    background.getCurrentTab().then(function (tab) {

        webPageCanvas.tabID = tab.id;

        if (tab.url.includes('file:///') || tab.url.includes('about://') || tab.url.includes('moz-extension://') || tab.url.includes('.pdf') || tab.url.includes('.png') || tab.url.includes('.jpg') || tab.url.includes('.gif') || tab.url.includes('.jpeg') || tab.url.includes('.JPEG') || tab.url.includes('.PNG') || tab.url.includes('.svg') || tab.url.includes('data:image/')) {
